...
if(wrapper.enabled)
{
        g.drawString("" + "Saved Points:", (((getSceneWidth()) / 2) - 62), ((getSceneHeight()) - ((getSceneHeight()) / 2.2)));
        g.drawString("" + Engine.engine.getGameAttribute("Store Points"), (((getSceneWidth()) / 2) - 2), (((getSceneHeight()) - ((getSceneHeight()) / 2.2)) + 16));
}
});
...
{
        switchScene(GameModel.get().scenes.get(0).getID(), createFadeOut(0, Utils.getColorRGB(0,0,0)), createFadeIn(0, Utils.getColorRGB(0,0,0)));
}
});
